...
private:
	...
	void copyList(const StringList& lst);
	    ...
	    // Stack class for undo actions
    class stack {
    public:
        stack();
        ~stack();
        void push(const std::string& command, int index, const std::string& value, const StringList& lst);
        void pop();
        void resize();
        void copyArrayHelper(string* dest, const string* src, int size);

        struct action {
            std::string command; // Type of operation (INSERT, REMOVE, etc.)
            int index;
            std::string value;   // Value to restore
            std::string* fullList; // Full list for undoing removeAll
            int fullListSize;    // Size of the full list
        };
